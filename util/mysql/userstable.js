...
class mysqlUsersTableService extends MySQLDataBase {
  constructor() {
    super();
  }
  ...
  async getUserPostsProfileByID(userid) {
    try {
      ...
      await this.getConnection();
      let userquery = "select user_id, user_name, CONCAT(?, user_imageid) AS user_imageurl, user_email from users where user_id = ?";
